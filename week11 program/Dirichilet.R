...
set.seed(0)
seed <- .Random.seed
X1 <- rdirichlet(1000, c(2,2,2))
X1 <- as.matrix(X1)
X <- as_tibble(X1)%>% rename("x1"="V1","x2"="V2","x3"="V3")
...
## Do not modify this line!
density_plot <- ggplot(data = X,aes(x1,x2)) + stat_density_2d(aes(fill = ..density..), geom="raster", contour = F)+
  geom_density_2d() + scale_x_continuous(expand = c(0,0))+ scale_y_continuous(expand = c(0,0)) +
  labs(title ="Density plot of Dir(2, 2, 2)", subtitle = "The samples are distributed in the lower left corner." )+
...
f<-function(x)
{
  if((x[1]+x[2]>1)|x[1]<0|x[2]<0){
    ...
    result=(gamma(6)/gamma(2)^3)*(x[1]*x[2]*(1-x[1]-x[2]))
  }
  return(result)
}
...
g<-function(x){
  if(x[1]<=1&x[2]<=1&x[1]>=0&x[2]>=0){return(1)}
  0}
...
#    - `theme_light()` to set the theme to light.
## Do not modify this line!
sample_plot <- ggplot(data = samples,aes(x=x1,y=x2))+ geom_point(alpha = 0.8) +
  labs(title = "Samples from Dir(2, 2, 2)",
       subtitle = "It is similar to ones that drawn from rdirichlet.") + theme_light()
sample_plot
...
test2 <- sample_n2(100, 100)

samples2 <- sample_n2(100)
acceptance_rate2 <- 100/sum(samples2$count)
