...
seed2 <- .Random.seed
x <- df %>% pull(x)
pvalue_result <-   ks.test(df %>% pull(x), pweibull,shape=3,scale=2 )$p.value
...
## Do not modify this line!
library(ggplot2)
d1 <- partial(dweibull,shape=3,scale=2)
g1 <- ggplot(df,aes(x= x)) + geom_histogram(bins = 40, aes(y  =..density..)) + stat_function(fun=d1,aes(colour = "True value"))+
...
## Do not modify this line!
g2 <- ggplot(df,aes(sample = x)) + geom_qq(distribution = qweibull, dparams = c(3,2))+
  stat_qq_line(distribution = qweibull,dparams = c(3,2),color = "red") + labs(
    title = "The sample seems to distribute along the straight line",
