...
library(tidyverse)
df <- read_csv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week11 program/patch.csv")%>%transmute(z= oldpatch - placebo, y =newpatch - oldpatch)
...
boot_ratio <- function(nboot,df){
  ...
  tib <- tibble(b=b,df_b=df_b, theta_hat_b=theta_hat_b)
  theta_hat_b = tib %>% pull(theta_hat_b)
  return(unlist(theta_hat_b))
}
...
set.seed(1)
seed1 <- .Random.seed
nboot <- c(50, 100, 200, 500, 1000)
theta_hat_b <- map(nboot,boot_ratio,df)
bias <-unlist(map(theta_hat_b,mean)) - theta_hat
theta_hat_corrected <- theta_hat - bias
type <- "normal"
results <- tibble(nboot, bias, theta_hat_corrected, type)
...
rratio <- function(p,df){
  result <- rmean(p,df$y)/rmean(p,df$z)
}
...
boot_ratio2<-function(nboot,df){
  t<-tibble(b=seq(1,nboot,1))%>%mutate(df_b=map(b,function(b,df)boot_p_ratio(df),df=df))%>%
    unnest(cols='df_b')%>%summarize(theta_hat_b=mean(theta),p_hat_b=list(mean_p(p)))}
...
results2 <- tibble(nboot = c(50, 100, 200, 500, 1000),
                   theta_hat_p_hat = map(nboot, ~boot_ratio2(.x, df))) %>% 
  unnest(theta_hat_p_hat) %>% 
  mutate(theta_hat = map_dbl(p_hat_b, ~ rratio(.x, df)),bias = theta_hat_b - theta_hat,theta_hat_corrected = theta_hat - bias,type = "improved") %>%
