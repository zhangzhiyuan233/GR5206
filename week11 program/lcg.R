...
g1 <- ggplot(unif_pairs,aes(x=X1,y=X2))+ geom_point()+
  labs(title = "Successive Pairs of Simulated X ~ Unif(0, 1)",
       subtitle = "Good generator as pairs are independent and uniformly distributed.",
       x = "X_n",
       y = "X_{n+1}") + theme_light()
g1
...
## Do not modify this line!
d1 <- partial(dexp,rate = 1.5)
g2 <- ggplot(exp_rng,aes(x=E)) + geom_histogram(aes(y=..density..),fill = "white", color = "black")+
  geom_density(aes(color = "Simulated"))+
  stat_function(fun= d1,aes(color = "Theoretical"))+
...
dnorm_density <- partial(dnorm,mean = 0, sd=1)
g3 <- norm_rng %>% gather(key = "key", value = "value", Z1,Z2)%>%
  ggplot(aes(x= value)) + geom_histogram(fill = "white", color = "black",aes(y= ..density..)) +
  geom_density(aes(color = "Simulated"))+
  stat_function(fun = dnorm_density ,aes(color = "Theoretical")) +
  facet_wrap(~key)+
