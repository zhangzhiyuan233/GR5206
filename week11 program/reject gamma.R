...
mode <- optimize(f1,c(0,10))$minimum
M_best <- f(mode)
...
test <- rejection_sampling(1, f, g, 0, 3)
...
## Do not modify this line!
n <- 1e3
len<-function(M,f,g,seed=0,n=1e3){
  set.seed(seed)
  y<-runif(n,0,10)
  u<-runif(n,0,1)
  p<-f(y)/(M*g(y))
  return(length(y[u<p])/n)}
result<-tibble(M=seq(M_best,5,0.025))%>%mutate(Y=map(M,rejection_sampling,f=f,g=g)) %>%
  mutate(acceptance_rate =map(Y,length)) %>% mutate(acceptance_rate = as.numeric(acceptance_rate)/1e3)

%>%mutate(acceptance_rate=map(M,len,f=f,g=g))%>%
  mutate(acceptance_rate=as.numeric(acceptance_rate))
...
## Do not modify this line!
acceptance_plot <- ggplot(result, aes(x = M, y =acceptance_rate)) +geom_line()+
  labs(x ="M", y="Acceptance rate", title = "The acceptance rate decreases with M") +
  theme_light()
#acceptance_plot
...
## Do not modify this line!
g2 <- function(x) {dnorm(x,mean = mode, sd =3)}
M2 <- f(mode)/g2(mode)
set.seed(1)
seed <- .Random.seed
n <- 1000
result2 <- tibble(y = rnorm(n,mode,3),u=runif(n,0,1)) %>% mutate(
...
g22 <- function(x) {M2*dnorm(x,mean = mode, sd =3)}

good_rejection_sampling_plot <- ggplot(result2,aes(x=y)) + geom_point(data = result2,aes(y=M2 * u * g2(y),color = selected ))+
  stat_function(fun=f,size =2)+ stat_function(fun =g22, color = "red", size=2) +
  labs(x="x",y="Density",title = "Acceptance illustration") + theme_light()
