...
m3 <- 3
m4 <- 13
m11 <- as.binary(2*0*13)
m21 <- as.binary(2^2*0*3)
m31 <- as.binary(2^3*1*1)
m41 <- as.binary(2^4*1*1)
m51 <- as.binary(1)
m5 <- m11 %>% xor(m21) %>% xor(m31) %>% xor(m41) %>% xor(m51) %>% as.numeric()
...
next_in_seq <- function(coeffs,previous){
  ...
  tib <- tibble(coeff_x_previous = coeff_x_previous, k = seq_along(coeff_x_previous)) %>%
    mutate(tmp  =map2(coeff_x_previous,k,aux))
  tmp <- tib %>% pull(tmp)
  result <- reduce(tmp,xor)
  final_result <- xor(result, as.binary(previous[length(previous)]))
  numeric_result <- as.numeric(final_result)
  return(numeric_result)
}
test <- next_in_seq(c(0, 1, 1), c(3, 1, 1))
