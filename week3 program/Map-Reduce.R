...
## Do not modify this line!
startime <-Sys.time()
mpls_mean <-mpls_num %>% map_dbl(mean) %>%as.list
endtime <-Sys.time()
...
## Do not modify this line!
sum1 <-reduce(mpls_num,sum)
sum2 <-sum(!is.na(mpls_num))
mpls_reduce <- sum1/sum2
...
subsum2 <-0
subsum3 <-0
for(i in 1:length(mpls_num$lat)){
  subsum1 <-subsum1+mpls_num$lat[i]
  subsum2 <-subsum2+mpls_num$long[i]
  subsum3 <-subsum3+mpls_num$policePrecinct[i]
}
sumtotal <- sum(subsum1,subsum2,subsum3)
naive_calculation <- sumtotal/sum(!is.na(mpls_num))
endtime <-Sys.time()
