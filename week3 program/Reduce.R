...
my_factorial <-function(n){
  if(n ==0 ){1}
    ...
    ans <-reduce(as.double(1:n),function(x,y){x*y})
...
r = my_rle(list(1,2,3,4,5,'a'), is.character)
...
span <- function(x, f){
  ...
  for (i in 1:length(r$lengths)){
    if(! any(r$values)) return(NA)
    else{
      longest <-max(r$lengths[r$values])
      index <-which(r$values & r$lengths ==longest)[1]
    }
    
}
      
}
span(list(1,2,3,4,5,'a'), is.character)
