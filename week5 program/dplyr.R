...
#      - `C` to arrange data set by sex with first `"F"` then `"I"` and `"M"`,
...
## Do not modify this line!
length_sex_ring <-abalone %>% as_tibble() %>% filter(length >0.6) %>% 
  arrange(sex,desc(rings)) %>% select(sex,diameter,height,rings)
...
#     shell_weight <dbl>
## Do not modify this line!
filter_na_rename_select_everything <- abalone %>% filter(diameter>0.36 | is.na(diameter)) %>% 
...
#    # . with 990 more rows
## Do not modify this line!
first_1000_rank <- abalone %>% select(diameter,rings) %>% mutate(rings_rank =row_number(rings)) %>% filter(rings_rank <=1000) %>% arrange(rings_rank)
