...
ea_na_75_85 <-longevity %>% filter((region =="East Asia & Pacific"|region =="North America"),(life_expect >=75 & life_expect <=85)) %>%
...
top_10_perc_us <- longevity %>% mutate(perc_rank = percent_rank(life_expect)) %>% arrange(desc(perc_rank)) %>% filter(perc_rank >=0.9|country =="United States")
...
gdp_by_region <-nations %>% mutate(gdp=gdp_percap*population) %>% group_by(region,year) %>% summarise(total_gdp = sum(gdp/1000000000000,na.rm = T))
...
count <-nations %>% filter(year ==2016) %>% group_by(income) %>%summarise(n = n())
