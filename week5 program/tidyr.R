...
t1 <- useR2016 %>% as_tibble() %>% select(Q2,Q3,Q7,Q8,Q11,Q13,Q13_B,Q13_C,Q13_D,Q13_E,Q13_F,Q25)
...
t1_longer <-t1 %>% pivot_longer(cols = starts_with("Q13"),names_to = "cases",values_to = "comments",values_drop_na =T)
...
#    # . with 1,757 more rows
## Do not modify this line!
separate_drop_redundant <-t1_longer_rename %>%separate(cases,into = c("case_name","category"),fill = "right") %>%
  replace_na(list(category = 'A')) %>% select(-"case_name")
...
experience_percentage <-t1_longer_rename %>% drop_na() %>% group_by(sex) %>%count(experience,name="count") %>%
  mutate(count =count / sum(count) * 100 ) %>% pivot_wider(names_from = experience,values_from = count)
