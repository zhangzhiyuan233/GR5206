...
library(dplyr)
library(readr)
activities <- read_csv("data/activities.csv") %>% as_tibble()
...
#    # . with 50 more rows
## Do not modify this line!
act_gathered <-activities %>% gather("work.T1","play.T1","talk.T1","work.T2","play.T2","talk.T2",key ="var",value = "score") %>%
  arrange(id,trt)
...
## Do not modify this line!
act_filled <- fill(act_spread,col_ix,.direction = "up")
