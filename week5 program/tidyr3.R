...
library(readr)
library(dplyr)
warpbreaks <- read_csv("data/warpbreaks.csv") %>% as_data_frame()
...
t1 <- warpbreaks %>% filter_all(all_vars(!is.na(.))) %>% filter(wool=="A"|wool=="B") %>%
  filter(tension =="L"|tension =="M"|tension =="H") %>% filter_all(any_vars(!.<0)) %>% filter_all(any_vars(!.>100))%>% 
  filter(!breaks==-999) %>% select(wool,tension,breaks)
...
t2 <- t1 %>% group_by(wool,tension) %>% count
...
t4 <- t3 %>% pivot_longer(-wool,names_to = "tension",values_to = "sum_of_breaks")
