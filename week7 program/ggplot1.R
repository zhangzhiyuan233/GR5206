...
library(readr)
library(tidyr)
abalone <- read_csv("C:/Users/zhang/OneDrive/Desktop/data/abalone.csv")
...
## Do not modify this line!
g <- ggplot(data = abalone,mapping = aes(rings,colour = sex))
g1 <- g+ stat_density(geom = "line",position  = "identity") +
 ...
 while infant abalone have lower mode in rings around 8",x= "Number of rings",y ="Density" ) + 
  scale_color_discrete(name = "Sex")+theme_light()+
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
...
## Do not modify this line!
g2 <- ggplot(abalone)+
  geom_histogram(mapping = aes(diameter,fill = sex),binwidth = 0.05 ) +
 facet_grid( ~ sex) +
  labs(title = "Female and male abalone tend to have more left skewed distributions of diameter,\nwhile the infant abalone\'s diameter distribution looks more normal",
       x = "Diameter of abalone (mm)", y = "Count of abalone") +scale_fill_discrete(name = "Sex")+theme_light()+
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
...
## Do not modify this line!
g3 <- ggplot(abalone) + geom_boxplot(mapping = aes(reorder(sex,-diameter),diameter))+
  labs(title ="Male and female abalone have similar median,\nwhile infant abalone have smaller median",x="Sex",y="Diameter of abalone (mm)" ) +
  theme_light()+
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
g3
...
## Do not modify this line!
g4 <- ggplot(abalone,mapping = aes(rings, diameter)) + geom_point() + geom_smooth(method = "loess",color = "red") + facet_wrap(~sex)+
  labs(title = "Diameter is increasing in rings",subtitle = "But the increase mostly stops around 10 rings",x = "Number of rings",
...
g5 <- ggplot(abalone,mapping = aes(length,whole_weight)) +geom_violin() + facet_wrap(~ sex) +
