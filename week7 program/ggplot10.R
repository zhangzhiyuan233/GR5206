...
library(readr)
library(dplyr)
taxi <- read_csv("C:/Users/zhang/OneDrive/Desktop/data/taxi.csv")
taxi <- taxi %>% filter(tip_amount < 15 & between(fare_amount,0,50))
taxi_head <-head(taxi)
...
library(scales)
library(ggplot2)
tip_histogram <- ggplot(taxi, aes(x= tip_amount)) +geom_histogram( binwidth = 1) +
  scale_y_continuous(label = comma) +labs(title = "The tip amounts are skewed to the right",
                                          x = "Tip Amount (USD)",
                                          y = "" ) + theme_light()
tip_histogram
...
taxi_binned <- taxi %>% mutate(fare_bins = cut(fare_amount, breaks = 5,labels = 
                                        c("0-10","11-20","21-30","31-40","41-50"))) 
...
tip_facet_hist <- ggplot(taxi_binned, aes(x=tip_amount))+geom_histogram(binwidth = 1) +scale_y_continuous(label = comma)+
  facet_grid(~fare_bins) + labs(title = "The tip skew becomes less severe as fares increase",
                                x= "Tip Amount (USD)", y ="") +theme_light()
...
library(hexbin)
hex_heatmap <- ggplot(taxi_binned, aes(x=fare_amount,y = tip_amount)) + 
  geom_hex(binwidth = c(2.5,0.5)) +scale_fill_gradient(low = "yellow", high = "darkgreen", trans = "log10")+
  labs(title = "There is a preference of tipping proportionately to the fare", x="Fare Amount (USD)",
