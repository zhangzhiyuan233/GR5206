...
wine <- read_csv("C:/Users/zhang/OneDrive/Desktop/data/winequalityN.csv")
wine <- drop_na(wine)
...
set.seed(0)
seed <- .Random.seed
plot1_1 <- ggplot(wine, aes(x = `fixed acidity`, y = quality)) +geom_point()+
  labs(title = "The quality does not seem to be correlated with acidity", x = "Fixed Acidity",
       y="Quality") 
plot1_2 <- ggplot(wine, aes(x = `fixed acidity`, y = quality)) + 
  geom_point(position = position_jitter()) +labs(title = "The pattern is easier to interpret with jitter",
...
plot2 <- ggplot(wine, aes(x = `fixed acidity`)) +
  ...
  facet_wrap(~quality) + labs(title = "Fixed acidity distribution for different quality levels",
...
## Do not modify this line!
library(GGally)
plot3 <- ggpairs(wine, mapping = aes(color = type), columns =c("density", "volatile acidity", "citric acid"),
...
library(plyr)
red = wine %>% filter(type == "red") %>% mutate(quality_label = cut(quality,c(2,5,7,10),right =T)) %>%
...
plot4 <- ggplot(red,aes(x=`citric acid`,y=density))+geom_smooth(method = "lm")+geom_point(aes(color = quality_label)) + scale_color_brewer(type = "qual") +
  labs(title = "Positive correlation of Density and Fixed Acidity",x = "Fixed Acidity",y = "Density")
plot4
