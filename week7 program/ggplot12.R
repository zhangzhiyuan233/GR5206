...
library(lubridate)
library(tidyverse)
fu <- read_csv("C:/Users/zhang/OneDrive/Desktop/data/football.csv")
fu1 <- fu %>% mutate(year = year(date)) %>% filter((home_team =="England"| home_team =="Germany" | home_team=="France" | home_team=="Belgium" | home_team=="Italy" | home_team =="Spain") &
                       ((year>2000)&(year < 2020)))%>% select(home_team,year,home_score) %>% group_by(home_team,year)%>% dplyr::summarise(total  = sum(home_score))
...
plot1 <- ggplot(fu1, aes(x = home_team, y = total, fill = home_team)) + geom_boxplot(alpha =0.6) +scale_fill_viridis(discrete = T) +
  labs(title = "Six European National Teams Home Goals from 2000 to 2019", x="Teams", y="Goals Overall (n)")
plot1
...
fu2 <- fu %>% dplyr::select(home_team,home_score,date) %>% mutate(year = year(date)) %>% filter(between(year,2014,2019)) %>% filter(home_team=="England")
...
library(ggridges)
plot2 <- ggplot(fu2, aes(x=year,y=home_score, group= home_score,fill = home_score)) + geom_density_ridges(alpha = 0.6) +
  labs(
         title = "England National Team Home Goals Distribution 
...
fu3 <- fu %>% select(home_team,tournament,date) %>%  
  ...
  drop_na() %>% filter((home_team =="England"| home_team =="Germany" | home_team=="France" | home_team=="Belgium" | home_team=="Italy" | home_team =="Spain"))  %>%
filter(year(date)>2000) %>% filter(tournament =="FIFA World Cup"| tournament=="UEFA Euro"| tournament=="UEFA Nations League"| tournament =="Nations Cup"| tournament =="Kirin Cup")
...
## Do not modify this line!
plot3 <- ggplot(fu3, aes(x=home_team, fill = tournament)) + geom_bar(position = "dodge", alpha = 0.75) +
  scale_fill_viridis(discrete=TRUE) + theme(axis.text.x = element_text(angle = 45)) +
  labs(title = "Six European National Team International Home Game Participation 
  
         Distribution from 2013 to 2019",
    subtitle = "Germany stands out in game numbers in World Cup participation",x = "Teams",
       y="Number of Home Games")
plot3
...
fu4 <- fu %>% filter(str_detect(tournament,"World Cup$") ==T) %>%
  pivot_longer(home_team:away_team ,names_to = "a", values_to = "team" ) %>% mutate(date = year(date)) %>%
  group_by(team) %>%distinct(date) %>% mutate(attendance = n()) %>% filter(attendance>8) %>%
  arrange(desc(attendance))  %>% rename(year=date)

fu5 <- fu %>% filter(str_detect(tournament,"World Cup$") ==T) %>%
  pivot_longer(home_team:away_team ,names_to = "a", values_to = "team" )
...
## Do not modify this line!
plot4 <- ggplot(fu4, aes(x=team),bins = 10) + geom_bar(fill ="lightsalmon1" ,color = "black",stat ="count") +
  coord_flip() + labs(title = "National Team Attendance Distribution in World Cup History",
