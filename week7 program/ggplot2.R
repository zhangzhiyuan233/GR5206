...
library(ggrepel)
library(readr)
autompg <- read_csv('C:/Users/zhang/OneDrive/Desktop/data/autompg.csv')
...
g1 <- ggplot(efficiency_by_manufacturer, mapping =aes(x=reorder(car_made,efficiency,median)  , y= efficiency),linetype=r2)+geom_boxplot() +
  coord_flip() + labs(title = "Cars made by European and Japanese manufacturers are generally more efficient",
...
## Do not modify this line!
g2 <- ggplot(efficiency_by_manufacturer, mapping = aes(x = weight, y = mpg,color= model_year)) + geom_point(size = 2, shape = 1, stroke = 1.25)+
  labs(x="Weight of a car (kg)",y ="Miles per gallon" ) +scale_color_discrete(name = "Model year of a car 
                                                                              (1970 - 1982)")+
  theme_light()+
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
...
## Do not modify this line!
g3 <-g2 + geom_smooth( method = "loess", se = T, color ="red") +guides()
g3
...
g4 <- ggplot(efficiency_by_manufacturer, linetype=r2) +
  geom_point(aes(x=weight, y=mpg,color=model_year), size = 2, shape=1, stroke=1.25) +
  geom_smooth(aes(weight, mpg),method = loess, se=T, color='red')+
  geom_text_repel(data=best_efficiency_by_manufacturer ,aes(x=weight, y=mpg,label=car_made))+
  labs(title = "Mpg is decreasing in weights",subtitle = "How are cars performing in best efficiency by different car manufacturers?",x="Weight of a car (kg)", y="Miles per gallon")+
  scale_color_discrete(name = "Model year of a car \n(1970 - 1982)")+
  theme_light()+
  guides(linetype=FALSE, col=guide_legend(nrow=1, override.aes = list(size=4)))+
  theme(legend.position="top", plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), legend.direction='horizontal', legend.justification=0.1)
