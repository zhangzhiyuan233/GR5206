...
library(ggplot2)
library(scales)
g1 <- ggplot(data = diamonds, mapping = aes(x =price,fill = cut )) + geom_histogram(binwidth = 500) +
  scale_x_continuous(label=comma) + scale_y_continuous(label=comma) +
  labs(title = "Counts go down as price goes up", subtitle = "Ideal cuts account for nearly half of the diamonds",
...
g2 <- ggplot(data = diamonds, mapping = aes(x =price, color = cut)) +geom_freqpoly(binwidth = 500)+
  labs(title = "Max count is reached when price is around 1000 USD", x="Price (USD)", y ="Count (n)")+
  theme_light()
g2
...
## Do not modify this line!
g3 <- ggplot(data =diamonds, mapping = aes(x = carat, y = price,color = cut)) +geom_point()+
  geom_smooth(se =F)+ labs(title = "In general price go up as carat goes up", subtitle = 
                             "Ideal cuts have highest increasing rate",x ="Carat", y="Price (USD)") +
  theme_light()
g3
...
## Do not modify this line!
g4 <- ggplot(data = diamonds,aes(x=carat , y= price, color = cut))+geom_point() +facet_grid(color~clarity)+
  labs(title = "In general price go up as carat goes up", subtitle = "I1 clarity seems to have the lowest increasing rate",
       x="Carat", y="Price (USD)") +theme_light()
g4
...
g5 <- ggplot(diamonds, aes(x=color, y = price)) + geom_boxplot() +scale_y_log10() +
  labs(title = "Color J has the highest price",x = "Color", y = "Price (USD)" )+theme_light()
g5
...
g6 <- ggplot(diamonds, mapping = aes(x=color, y = price)) + geom_violin()+scale_y_log10()+facet_wrap(~clarity)+
  labs(title = "Diffrent clarities and colors show various distribution in price",x = "Color",y = "Price (USD)")+theme_light()
g6
