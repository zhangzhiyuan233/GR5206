...
data(iris)
library(ggplot2)
g1 <- ggplot(iris) + geom_point(mapping = aes(x=Sepal.Length,y=Petal.Length,color=iris$Species)) +
  labs(title = "Sepal length by petal length",x = "Sepal Length (cm)",y = "Petal Length (cm)")+
  theme_light()
g1
...
g2 <- g1 +theme(plot.title = element_text(hjust=0.5)) +scale_color_hue(name = "Species")
g2
...
g3 <- g2 + geom_smooth(mapping = aes(x=Sepal.Length,y=Petal.Length,color=iris$Species),method = "lm",se=F,fullrange = TRUE)+
  labs(title = "Petal length is increasing in sepal length",subtitle="Except for the setosa species")+theme(plot.subtitle = element_text(hjust=0.5))
g3
...
g4 <- g3 + geom_point(mapping = aes(x=Sepal.Length[15],y=Petal.Length[15]),pch =17, col = "black", cex = 3)
g4
...
g5 <- g4 + geom_text(mapping = aes(x=Sepal.Length[15],y=Petal.Length[15]), label = "This is the iris with the longest sepal\n among the setosa species.",
                     col = "black", nudge_x = 1)
g5
