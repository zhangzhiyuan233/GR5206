...
library(tidyverse)
nations1 <- read_csv("C:/Users/zhang/OneDrive/Desktop/data/nations.csv")
nations <- nations1 %>% drop_na() %>% mutate(region = fct_reorder(region,life_expect))
...
## Do not modify this line!
g1 <- ggplot(nations,mapping = aes(region,life_expect))+geom_boxplot()+coord_flip()+
  labs(title = "Life Expectancy by Region",x = "Region",y = "Life expectancy (years)")+theme_light()
g1
...
g2 <- g1 + geom_hline(aes(yintercept = med),color = "red") + labs(subtitle = paste0("Overall median = ", round(med), " years (shown in red)"))
g2
...
g3 <- ggplot(nations,aes(x=gdp_percap,y =life_expect))+geom_point(alpha = 0.5, stroke = 0) + labs(title = "Life Expectancy Increases with GDP Per Capita",
...
## Do not modify this line!
g4 <- g3 + scale_x_log10() + geom_smooth(se=F)
g4
