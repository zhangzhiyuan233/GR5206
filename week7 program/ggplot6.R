...
library(tidyverse)
library(viridis)
taxi <- read_csv("C:/Users/zhang/OneDrive/Desktop/data/nyc_taxi.csv")
...
## Do not modify this line!
g1 <- ggplot(taxi, aes(x=fare_amount, y =tip_amount ))+geom_point(alpha = 0.5, stroke = 0)+coord_cartesian(xlim = c(0, 60),ylim = c(0, 15))+
  labs(title = "Tip Amount Increases with Fare Amount Generally",subtitle = "Some people gave fixed tips regardless of fares.",
       x = "Fare amount ($)", y = "Tip amount ($)") +theme_light()
g1
...
g2 <- ggplot(taxi, aes(x=fare_amount, y=tip_amount))+geom_bin2d(binwidth = c(2, 1))+coord_cartesian(xlim = c(0, 60), ylim = c(0, 15)) +
  scale_fill_viridis() + labs(title = "Tip Amount Increases with Fare Amount Generally",
                              subtitle = "Most commonly, trips had fares under $10 and tips around $2.",
                              x = "Fare amount ($)",
...
## Do not modify this line!
g3 <- ggplot(taxi,aes(x=trip_distance, y =tip_amount, color = payment_type)) + geom_point(alpha = 0.5, stroke = 0) +
  coord_cartesian(xlim = c(0, 25),ylim = c(0, 15)) + labs(title = "Tip Amount Increases with Trip Distance Generally",
                                                          subtitle = paste("Trips with tips were almost always paid in card.",
                                                        "Trips paid in cash were rarely tipped.", sep="\n") ,
                                                        x = "Trip distance (mile)", 
                                                        y="Tip amount ($)"
...
## Do not modify this line!
 a <- format(taxi$tpep_pickup_datetime, format = "%H")

 taxi_new <- taxi %>% mutate(pickup_time = format(tpep_pickup_datetime, format = "%H") ) %>%
...
g4 <- ggplot(taxi_new, aes(x=pickup_time, y=tip_amount))+ stat_summary(fun.ymin = "min",fun.ymax = "max",fun.y = "median") +
  coord_cartesian(ylim = c(0, 50))+ labs(title = "Min/Max/Median Tip Amount by Pickup Time",
                                         subtitle = paste("The highest tip was paid to a trip at 4AM.",
                                                          "9PM had the highest median tip whereas 8PM/10PM had the lowest.",
                                                          sep="\n"),
                                         x = "Pickup Time (hour)",
