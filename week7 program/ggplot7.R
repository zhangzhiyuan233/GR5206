...
#    # . with 17 more rows
## Do not modify this line!
mean_value <- wine %>% select(-Type) %>% dplyr::summarize_all(mean) %>%
  pivot_longer(col = everything(),
  names_to = "Property",values_to = "Value") %>%
  mutate(Property = fct_reorder(Property, Value)) %>% mutate(Property=as.factor(Property))
...
## Do not modify this line!
mean_value_plot <- ggplot(mean_value, aes(x = Value, y = Property)) + geom_point() +scale_x_log10() +
  ggtitle(label = "Most properties have value below 100") +theme_light()
...
mean_value_by_type <- wine %>% group_by(Type) %>% summarize_all(mean) %>% 
  pivot_longer(-Type, names_to = "Property", values_to = "Value") 

vector <-  mean_value_by_type %>% group_by(Property) %>% summarise(m= max(Value)) %>% arrange(m) %>%pull(1)

mean_value_by_type <- mean_value_by_type %>% mutate(Property = fct_relevel(as.factor(Property),vector))
...
## Do not modify this line!
mean_value_plot2 <- ggplot(mean_value_by_type, aes(x=Value, y=Property,color = factor(Type)))+ geom_point()+
  scale_x_log10() + labs(color = "Type") + ggtitle("Mean Proline values vary significantly among three types")+
  theme_light()
mean_value_plot2
