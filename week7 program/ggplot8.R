...
library(readr)
library(scales)
fifa <- read_csv("C:/Users/zhang/OneDrive/Desktop/data/fifa.csv")
...
age_plot <- ggplot(fifa, aes(x=Age)) +geom_histogram(aes(x=Age, y=..density..),binwidth = 2,color = "white") +geom_density(color = "blue")+
  labs(title = "The majority of players are 20 to 30 years old",y = "Density") +theme_light()
age_plot
...
## Do not modify this line!
club_wage <- fifa %>% filter(Club =="FC Barcelona"| Club =="Juventus" | Club =="Real Madrid"|
                               Club =="Manchester United"| Club =="Manchester City") %>%
...
wage_plot <- ggplot(club_wage,aes(x= Club, y = Wage)) +geom_boxplot() +scale_y_continuous(labels = dollar_format(suffix = "???", prefix = "")) +
  labs(title = "Top 5 highest median wage clubs",subtitle = "The medians are remarkably similar,but the higher quantiles show more variation.",y = "Wage",x = "")+coord_flip() +theme_light()
wage_plot
...
fifa_height_weight <- fifa %>% filter(Position =="GK"| Position =="CM") 
...
height_weight_plot <- ggplot(data = fifa_height_weight,aes(x = Height_m, y =Weight_kg, color = Position))+
  geom_point(alpha = 0.8) + labs(title = "Goalkeepers tend to be heavier and taller", x="Height (m)",
                                 y="Weight (kg)") +theme_light()
height_weight_plot
...
agility <- fifa %>% filter(fifa$`Preferred Foot` == "Left"|fifa$`Preferred Foot` == "Right")
...
## Do not modify this line!
agility_plot <- ggplot(agility,aes(x =Agility))+geom_histogram(bins = 20,color="white") + facet_wrap(~ `Preferred Foot`, scales = "free") +
  labs(title = "Distribution of agility is similar regardless of preferred foot",y = "Count (n)") + theme_light()
