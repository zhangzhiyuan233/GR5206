...
library(readr)
library(dplyr)
employee <- read_csv("C:/Users/zhang/OneDrive/Desktop/data/employee.csv")
employee <- employee %>% select(-1) %>% mutate(Year = as.factor(Year))
...
library(ggplot2)
library(scales)
salary_boxplots <- ggplot(employee, aes(x=Year, y=Salaries)) +geom_boxplot() +scale_y_continuous(label = comma) +
  labs(title = "Salaries are getting a longer tail with time", y = "Yearly salaries (USD)") +theme_light()
salary_boxplots
...
employee_trunc <- employee %>% mutate(Salaries = ifelse(Salaries >250000, 250000, Salaries ))
...
salary_histograms <- ggplot(employee_trunc) + geom_histogram(aes(x=Salaries),binwidth = 5000) +facet_wrap(~Year) +scale_x_continuous(label = comma)+
  scale_y_continuous(label = comma) +labs(title = "Salaries have a slowly changing bimodal distribution",x = "Yearly salaries (USD)", y = "")  + theme_light()+
...
salary_densities <- ggplot(employee_trunc, aes(x = Salaries, color = Year)) +geom_density() +
  scale_x_continuous(label = comma) + labs(title = "The salaries modes are becoming less acute with time",
                    x = "Yearly salaries (USD)",      y = "Densities for each year" ) +
  theme_light()
salary_densities
...
employee_culture <- employee %>% filter(organization =="Culture & Recreation" & Salaries>0)
...
#    - finally add the light theme using `theme_light()`
## Do not modify this line!
log_salaries_qq <- ggplot(employee_culture,aes( sample =log10(x=Salaries) )) + geom_qq() +
  geom_qq_line() +labs(title = "The log salaries severely diverge from a normal distribution",
  x = "Theoretical quantiles of log(salaries)" , y = "Sample quantiles of log(salaries)") + theme_light()
log_salaries_qq
