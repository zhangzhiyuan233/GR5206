...
library(readr)
trips <- read.csv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week8 program/data/trips.csv")
...
## Do not modify this line!
library(dplyr)
trips_with_genders <- trips %>% mutate(gender = factor(gender,levels = c(0,1,2),labels = c("Unknown","Male","Female")))
...
## Do not modify this line!
library(lubridate)
trips_with_dates <- trips %>% mutate(starttime = mdy_hms(starttime), stoptime =mdy_hms(stoptime))
...
trips_per_hour <- trips_start_times %>% group_by(start_hour) %>% summarise(num_trips = n(),
                    num_days =n_distinct(day(starttime)), mean_trips = num_trips/ num_days)
