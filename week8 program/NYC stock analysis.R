...
library(dplyr)
library(tidyr)
raw <- read_csv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week8 program/data/prices.csv")
securities <- read_csv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week8 program/data/securities.csv")
fund <- read_csv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week8 program/data/fundamentals.csv")
...
securities_selected <- securities %>% anti_join(securities_sectored)
...
fund_time <- fund %>% dplyr::rename("Ticker symbol" ="Ticker Symbol") %>% mutate(`Period Ending Year` =
    year(mdy(str_replace_all(`Period Ending`,"/","-"),tz =  "America/New_York"    )) )  %>% drop_na() %>%
...
## Do not modify this line!
securities_fund <- securities_selected %>% dplyr::select(`Ticker symbol`,`Security`,`GICS Sector`) %>%
...
gross_margin <- ggplot(securities_fund, aes(x =`Gross Margin`, fill =`Period Ending Year`   ))+
  geom_histogram(binwidth = 10,color = "black", fill = "orange") + facet_wrap(~`GICS Sector`) +
...
full_stock <- raw_time %>% rename("Ticker symbol" = "symbol") %>% mutate(year = year(date)) %>%
...
## Do not modify this line!
filtered_company <- full_stock %>% filter(Security %in% c("Aetna Inc", "Amazon.com Inc", "Facebook", "Whole Foods Market",
                                                          "FedEx Corporation", "Boeing Company", "The Walt Disney Company"  ))
...
trend <- ggplot(filtered_company,aes(x= date, y = close)) + geom_line(aes(color = Security)) +
  labs(title = "Six company daily stock close price from 2010 ~ 2016",
       x = "date",y = "Daily close price (USD)" ) + theme_light()
...
return_stock <- full_stock %>% group_by(`Period Ending Year`,`Ticker symbol`) %>% filter(date == min(date) | date == max(date)) %>%
  ...
  filter(date == status) %>% summarise(Return = diff(price)/price[1]) %>% left_join(securities_fund,by =c("Ticker symbol", "Period Ending Year") ) %>%
  droplevels()
...
## Do not modify this line!
summary_stock <- return_stock %>% group_by(`GICS Sector`) %>% summarise(mean_return = mean(Return),q1 = quantile(Return, probs = 0.25),q2 = quantile(Return, probs = 0.75))%>%
