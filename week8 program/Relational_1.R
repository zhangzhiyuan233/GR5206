...
library(readr)
terms_mentions <- read_tsv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week8 program/data/mentions_yearly_counts.tsv",
...
#    # . with 415 more rows
## Do not modify this line!
total_mentions <- read_csv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week8 program/data/yearly_total_counts.csv",
                           col_names = c("year","total_mentions","total_page_count","total_book_count"),
                           col_types = c(col_integer(),col_double(),col_integer(),col_integer()))
...
## Do not modify this line!
library(dplyr)
mentions <- terms_mentions %>% left_join(total_mentions , by = "year")
...
diagnosis <- anti_join(terms_mentions,mentions,by = c("term", "year", "n_mentions", "book_count"))
...
library(forcats)
examples_mention_counts <- relative_mention_counts %>% filter(term == 1883|term == 1910|term == 1950) %>%
...
library(ggplot2)
library(scales)
paper_figure <- ggplot(examples_mention_counts,aes(x=year,y=frac_total,color = term)) +geom_line() +
  scale_y_continuous(label = percent) + coord_cartesian(xlim = c(1850,2012)) +
