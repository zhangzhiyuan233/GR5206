...
library(tidyverse)
library(lubridate)
movies <- read_csv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week8 program/data/movies.csv")
ratings <- read_csv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week8 program/data/ratings.csv")
...
#    # . with 49,990 more rows
## Do not modify this line!
ratings <- ratings %>% mutate(date = as.POSIXct(timestamp, origin ="1970-01-01")) %>% mutate(date = 
                                                                                               with_tz(date,
                                                                                                       tzone = "UTC"         ))
...
average_rating_in_different_years <- ratings %>% mutate(year_of_rating = as.factor(year(date))) %>%
  group_by(year_of_rating) %>% summarise(average_rating_of_year = mean(rating))
...
g1 <- ggplot(average_rating_in_different_years,aes(x =year_of_rating, y =average_rating_of_year))+geom_point()+
  labs(title = "2004 and 2005 have lower average ratings",subtitle = "While 2014 has highest average ratings",
       x= "Year", y = "Average rating") + theme_light()+theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
g1
...
movies <- movies %>% mutate(year = map_chr(substr(title,nchar(movies$title)-4,nchar(movies$title)-1) ,as.integer)) %>%
...
movies <- movies %>% mutate(genres = as.character(genres))  %>%separate_rows(genres)
...
ratings_of_movies <- movies %>% inner_join(ratings, by = "movieId") %>% select(-movieId,-timestamp,-userId)
...
g2 <- ggplot(ratings_of_movies,aes(x = genres, y = rating)) + geom_boxplot()+
  labs(title ="Most movie genres have median ratings from 3.5 to 4",x = "Genres",
       y = "Ratings") + coord_flip() + theme_light()+theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
g2
...
#    # . with 10 more rows
## Do not modify this line!
average_rating_of_genres <- ratings_of_movies %>%  group_by(genres) %>% summarise(average_rating = 
       mean(rating), number_of_ratings = n()) %>% mutate(genres = as.factor(genres))
...
g3 <- ggplot(average_rating_of_genres,aes(x = number_of_ratings, y =average_rating)) +geom_point(aes(color = genres))+
  geom_smooth(method = "loess") + labs(title = "When number of ratings is over 5000, the average ratings start to be constant around 3.5",
  x ="Number of ratings",y ="Average rating",color = "Genres" )+ theme_light() +theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
...
ratings_over_time <- ratings_of_movies %>% mutate(interval = difftime(date,make_datetime(year),units = "days"))%>%mutate(years_passed=interval/dyears())%>%
...
g4 <- ggplot(ratings_over_time,aes(x=years_passed, y =rating, color = genres)) +
  geom_smooth(method = "lm",se = F) + labs(title = "The average rating usually increases with time",
                                           subtitle = "One exception is animation movies",
                                    x = "Years between release and rating",
                                    y="Average rating",
                                    color = "Genres") +theme_light() +theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
