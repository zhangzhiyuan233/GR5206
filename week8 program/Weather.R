...
#'
## Do not modify this line!
weather_9am <- weather %>% select(Date, ends_with("9am")) %>% rename_all(~str_remove(.x,"9am")) %>%
  mutate(Date = ymd_hms(paste(Date,"09:00:00"),tz="Australia/Canberra"))
weather_3pm <- weather %>% select(Date,ends_with("3pm")) %>% rename_all(~str_remove(.x,"3pm")) %>%
...
#'
## Do not modify this line!
weather_arrange <- full_join(weather_9am,weather_3pm, by = colnames(weather_3pm)) %>%
  mutate(Date = yday(Date))  %>% rename( "Day of the year"= Date ) %>% arrange(`Day of the year`)
...
## Do not modify this line!
temp_humidity_plot <- ggplot(weather_arrange, aes(x= `Day of the year`)) +geom_smooth(aes(y=Humidity),col ="green",
                                                                                      method = "loess",formula ='y ~ x')+
  geom_smooth(aes(y=3*Temp),col = 'yellow',method = "loess", formula = 'y~x')+
  labs(title = "Temperature and Humidity are negatively correlated in Canberra",x="Day of the year",
       y="Humidity (in green) and rescaled Temperature (in yellow)")+theme_light()
temp_humidity_plot
...
weather_wind <- full_join(weather_wind_9am,weather_wind_3pm,by = colnames(weather_wind_9am)) %>%
  drop_na() %>% mutate(WindMainDir = factor(str_sub(WindDir,end =1) )) %>%
  arrange(Temp)
str(weather_wind)
...
## Do not modify this line!
wind_histogram <- ggplot(weather_wind,aes(x=Temp)) + geom_histogram(color = "black", fill = "orange",binwidth = 0.5)+
  geom_vline(aes(xintercept=29),color = "green") + facet_wrap(~WindMainDir)+labs(
    title = "The winds going west and north bring the highest temperatures",
