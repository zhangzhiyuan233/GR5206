...
library(purrr)
library(tibble)
people <- read_csv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week8 program/data/people.csv")
email <- read_csv("C:/Users/zhang/OneDrive/Desktop/Introduct to DS/week8 program/data/email.csv")
...
library(dplyr)
library(lubridate)
email_w_time <- email %>% mutate(date = with_tz(as.POSIXct((onset -883612800), origin ="1998-01-01" ),tzone = "UTC" )) 
...
email_id_to_name <- function(email_id){
  id <- str_split(email_id, "\\.") %>% map_chr(partial(paste0),collapse = " ") %>% str_trim() %>% str_to_title()
}
...
people_new2 <- people_new %>% mutate(role = replace_na(role,"Employee"), dept = replace_na(dept,"General")) %>%
...
t1<-left_join(email_w_time,people_new2,by=c("tail"="vertex.names"))%>%dplyr::rename(receiver_email=email_id,receiver=person_name,receiver_role=role,receiver_dept=dept)
t1<-left_join(t1,people_new2,by=c("head"="vertex.names"))%>%dplyr::rename(sender_email=email_id,sender=person_name,sender_role=role,sender_dept=dept)%>%
...
t2 <- t1 %>% filter(sender_email != receiver_email) %>% mutate(year = year(date),
                                                               month = month(date),
                                                               day = day(date),
                                                               hour = hour(date))
...
p1 <- t2 %>% group_by(sender,sender_role,sender_dept) %>% summarise(count = n())%>%arrange(desc(count))%>%head(3)
