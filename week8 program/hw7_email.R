...
library(purrr)
library(tibble)
people<-read_csv("data/people.csv")%>%as_tibble()
email<-read_csv("data/email.csv")%>%as_tibble()
...
library(dplyr)
library(lubridate)
email_w_time<-email%>%mutate(date=as.POSIXct(onset-883612800,origin="1998-01-01"))%>%
...
email_id_to_name<-function(email_id){
  output<-str_split(email_id,"\\.")
  output<-map_chr(output,~paste0(.x,collapse=" "))%>%str_trim()%>%str_to_title()
  return(output)
}
...
people_new2$role<-people_new2$role%>%replace_na("Employee")
people_new2$dept<-people_new2$dept%>%replace_na("General")
people_new2<-people_new2%>%mutate(role=factor(role,levels=role_order))
people_new2
...
## Do not modify this line!
t1<-left_join(email_w_time,people_new2,by=c("tail"="vertex.names"))%>%rename(receiver_email=email_id,receiver=person_name,receiver_role=role,receiver_dept=dept)
t1<-left_join(t1,people_new2,by=c("head"="vertex.names"))%>%rename(sender_email=email_id,sender=person_name,sender_role=role,sender_dept=dept)%>%
...
## Do not modify this line!
t2<-t1%>%filter(!(sender_email==receiver_email))%>%mutate(year=year(date),month=month(date),day=day(date),hour=hour(date))
t2
...
#    <chr>           <fct>          <chr>              <int>
## Do not modify this line!
p1<-t2%>%group_by(sender,sender_role,sender_dept)%>%summarize(count=n())%>%arrange(desc(count))%>%head(3)
p1
...
## Do not modify this line!
library(ggplot2)
g1<-ggplot(data=t2,aes(x=hour))+geom_histogram(bins=24)+
...
## Do not modify this line!
p2<-t2%>%filter(date<ymd("2002/01/01"))%>%group_by(year,month,sender_role)%>%summarize(date=min(date),count=n())%>%arrange(date)
p2
...
#'
## Do not modify this line!
g2<-ggplot(data=p2,aes(x=date,y=count))+geom_point(aes(color=sender_role))+geom_smooth(aes(color=sender_role))+
  labs(title = "People are using emails more frequently in 2001 than 1999",x = "Date",y = "Count(n)",color = "Sender Role")+
  theme_light()+theme(plot.title=element_text(hjust = 0.5))
g2
