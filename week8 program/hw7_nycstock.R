...
#    # 鈥? with 851,254 more rows
#    `securities` should look like:
#    # A tibble: 505 x 8
#    `Ticker symbol` Security `SEC filings` `GICS Sector`
#    <chr>           <chr>    <chr>         <chr>
#    1 MMM             3M Comp鈥? reports       Industrials
#    2 ABT             Abbott 鈥? reports       Health Care
#    3 ABBV            AbbVie   reports       Health Care
#    4 ACN             Accentu鈥? reports       Information 鈥?
#    5 ATVI            Activis鈥? reports       Information 鈥?
#    6 AYI             Acuity 鈥? reports       Industrials
#    7 ADBE            Adobe S鈥? reports       Information 鈥?
#    8 AAP             Advance鈥? reports       Consumer Dis鈥?
#    9 AES             AES Corp reports       Utilities
#    10 AET             Aetna I鈥? reports       Health Care
#    # 鈥? with 495 more rows, and 4 more variables: `GICS Sub
#    #   Industry` <chr>, `Address of Headquarters` <chr>, `Date first
#    #   added` <date>, CIK <chr>
#    `fund` should print to:
#    # A tibble: 1,781 x 79
#    X1 `Ticker Symbol` `Period Ending` `Accounts Payab鈥?
#    <dbl> <chr>           <date>                     <dbl>
...
#    # 鈥? with 1,771 more rows, and 75 more variables: `Accounts
...
library(dplyr)
library(tidyr)
raw<-read_csv("/course/data/prices.csv")%>%as_tibble()
securities<-read_csv("data/securities.csv")%>%as_tibble()
fund<-read_csv("data/fundamentals.csv")%>%as_tibble()
...
#    2 APD             Air Pro鈥? reports       Materials
#    3 ALB             Albemar鈥? reports       Materials
#    4 LNT             Alliant鈥? reports       Utilities
#    5 AEE             Ameren 鈥? reports       Utilities
#    6 AEP             America鈥? reports       Utilities
#    7 AWK             America鈥? reports       Utilities
#    8 APC             Anadark鈥? reports       Energy
#    9 APA             Apache 鈥? reports       Energy
#    10 T               AT&T Inc reports       Telecommunic鈥?
#    # 鈥? with 84 more rows, and 5 more variables: `GICS Sub
#    #   Industry` <chr>, `Address of Headquarters` <chr>, `Date first
...
#    7 ADBE            Adobe S鈥? reports       Information 鈥?
#    8 AAP             Advance鈥? reports       Consumer Dis鈥?
#    9 AET             Aetna I鈥? reports       Health Care
#    10 AMG             Affilia鈥? reports       Financials
#    # 鈥? with 401 more rows, and 4 more variables: `GICS Sub
#    #   Industry` <chr>, `Address of Headquarters` <chr>, `Date first
#    #   added` <date>, CIK <chr>
## Do not modify this line!
library(forcats)
securities<-securities%>%mutate(`GICS Sector`=factor(`GICS Sector`,levels=unique(`GICS Sector`)))
securities_sectored<-securities%>%mutate(`GICS Sector truncated`=fct_lump(fct_infreq(`GICS Sector`),6))%>%
  filter(`GICS Sector truncated`=="Other")%>%filter(!(str_detect(`GICS Sub Industry`,"Gold")|str_detect(`GICS Sub Industry`,"REITs")))
securities_selected<-anti_join(securities,securities_sectored)
...
#    9 AAPL                            2013             38
#    10 AAPL                            2014             39
#    # 鈥? with 1,289 more rows
## Do not modify this line!
fund_time<-fund%>%rename(`Ticker symbol`=`Ticker Symbol`)%>%
  mutate(`Period Ending Year`=year(mdy(str_replace_all(`Period Ending`,"\\/","\\-"),tz="America/New_York")))%>%
  drop_na()%>%select(`Ticker symbol`, `Period Ending Year`, `Gross Margin`)
fund_time
...
#    # A tibble: 988 x 5
#    `Ticker symbol` Security `GICS Sector` `Period Ending 鈥?
#    <chr>           <chr>    <fct>                    <dbl>
#    1 MMM             3M Comp鈥? Industrials               2013
#    2 MMM             3M Comp鈥? Industrials               2014
#    3 MMM             3M Comp鈥? Industrials               2015
#    4 ABT             Abbott 鈥? Health Care               2012
#    5 ABT             Abbott 鈥? Health Care               2013
#    6 ABT             Abbott 鈥? Health Care               2014
#    7 ABT             Abbott 鈥? Health Care               2015
#    8 ABBV            AbbVie   Health Care               2013
#    9 ABBV            AbbVie   Health Care               2014
#    10 ABBV            AbbVie   Health Care               2015
#    # 鈥? with 978 more rows, and 1 more variable: `Gross Margin` <dbl>
## Do not modify this line!
securities_fund<-securities_selected%>%select(`Ticker symbol`, `Security`, `GICS Sector`)
securities_fund<-inner_join(securities_fund,fund_time)
securities_fund
...
## Do not modify this line!
library(ggplot2)
gross_margin<-ggplot(data=securities_fund,aes(x=`Gross Margin`,fill=`Period Ending Year`))+
  geom_histogram(binwidth=10,color="black",fill="orange")+facet_wrap(vars(securities_fund$`GICS Sector`))+
  labs(title = "Gross margin distributed differently in different sectors",x = "Gross Margin (%)",y = "Count (n)")+
  theme_light()
gross_margin
...
#    1 AAL               5.12   5.2  2012-01-03 00:00:00  2012 America鈥?
#    2 AAP              69.1   71.1  2012-01-03 00:00:00  2012 Advance鈥?
#    3 ABT              56.7   56.6  2012-01-03 00:00:00  2012 Abbott 鈥?
#    4 ADS             103.   103.   2012-01-03 00:00:00  2012 Allianc鈥?
#    5 AKAM             32.9   33.0  2012-01-03 00:00:00  2012 Akamai 鈥?
#    6 ALK              73.9   76.4  2012-01-03 00:00:00  2012 Alaska 鈥?
#    7 AME              42.2   43.3  2012-01-03 00:00:00  2012 AMETEK 鈥?
#    8 AMT              58.8   60.5  2012-01-03 00:00:00  2012 America鈥?
#    9 APH              46.0   46.5  2012-01-03 00:00:00  2012 Ampheno鈥?
#    10 ARNC              9.23   8.94 2012-01-03 00:00:00  2012 Arconic鈥?
#    # 鈥? with 246,605 more rows, and 3 more variables: `GICS
#    #   Sector` <fct>, `Period Ending Year` <dbl>, `Gross Margin` <dbl>
## Do not modify this line!
full_stock<-rename(raw_time,`Ticker symbol`=symbol)
full_stock<-mutate(full_stock,year=year(date))%>%
  select(`Ticker symbol`, close, open, date,year)
full_stock<-left_join(full_stock,securities_fund,by="Ticker symbol")%>%filter(`Period Ending Year`==year)
full_stock
...
#    `Ticker symbol` close  open date                 year Security
#    <chr>           <dbl> <dbl> <dttm>              <dbl> <chr>
#    1 AMZN            257.  256.  2013-01-02 00:00:00  2013 Amazon.鈥?
#    2 BA               77.1  76.6 2013-01-02 00:00:00  2013 Boeing 鈥?
#    3 DIS              51.1  50.8 2013-01-02 00:00:00  2013 The Wal鈥?
#    4 FB               28    27.4 2013-01-02 00:00:00  2013 Facebook
#    5 FDX              94.2  93.5 2013-01-02 00:00:00  2013 FedEx C鈥?
#    6 WFM              92.0  93.1 2013-01-02 00:00:00  2013 Whole F鈥?
#    7 AMZN            258.  257.  2013-01-03 00:00:00  2013 Amazon.鈥?
#    8 BA               77.5  77.0 2013-01-03 00:00:00  2013 Boeing 鈥?
#    9 DIS              51.2  51.0 2013-01-03 00:00:00  2013 The Wal鈥?
#    10 FB               27.8  27.9 2013-01-03 00:00:00  2013 Facebook
#    # 鈥? with 5,282 more rows, and 3 more variables: `GICS Sector` <fct>,
#    #   `Period Ending Year` <dbl>, `Gross Margin` <dbl>
## Do not modify this line!
filtered_company<-full_stock%>%filter(Security%in%c("Aetna Inc", "Amazon.com Inc", "Facebook", "Whole Foods Market","FedEx Corporation", "Boeing Company", "The Walt Disney Company"))
filtered_company
...
## Do not modify this line!
trend<-ggplot(data=filtered_company,aes(x=date,y=close))+geom_line(aes(color=Security))+
  labs(title = "Six company daily stock close price from 2010 ~ 2016",x = "date",y = "Daily close price (USD)")+
  theme_light()
...
#     `Period Ending 鈥? `Ticker symbol`  Return Security `GICS Sector`
#     <dbl> <chr>             <dbl> <chr>    <fct>
#     1             2012 AAL              1.60   America鈥? Industrials
#     2             2012 AAP              0.0170 Advance鈥? Consumer Dis鈥?
#     3             2012 ABT              0.158  Abbott 鈥? Health Care
#     4             2012 ADS              0.406  Allianc鈥? Information 鈥?
#     5             2012 AKAM             0.241  Akamai 鈥? Information 鈥?
#     6             2012 ALK             -0.436  Alaska 鈥? Industrials
#     7             2012 AME             -0.133  AMETEK 鈥? Industrials
#     8             2012 AMT              0.278  America鈥? Real Estate
#     9             2012 APH              0.390  Ampheno鈥? Information 鈥?
#     10             2012 ARNC            -0.0291 Arconic鈥? Industrials
#     # 鈥? with 970 more rows, and 1 more variable: `Gross Margin` <dbl>
## Do not modify this line!
return_stock<-full_stock%>%group_by(`Period Ending Year`,`Ticker symbol`)%>%filter(date==min(date)|date==max(date))%>%
  mutate(date=ifelse(date==min(date),"open","close"))%>%pivot_longer(c("open","close"),names_to="status",values_to="price")%>%
  filter(date==status)%>%summarize(Return=diff(price)/price[1])
return_stock<-left_join(return_stock,securities_fund,by=c("Ticker symbol", "Period Ending Year"))%>%droplevels()
...
summary_stock<-return_stock%>%group_by(`GICS Sector`)%>%
  summarize(mean_return=mean(as.numeric(Return)),q1=quantile(as.numeric(Return),probs=0.25),q2=quantile(as.numeric(Return),probs=0.75))
summary_stock<-summary_stock%>%mutate(`GICS Sector`=fct_reorder(`GICS Sector`,mean_return))
summary_stock
